...
- class难以重用和维护，逻辑代码更为复杂。
- class状态难以共享。
- 类组件中的this增加学习成本。

例如，一个简单的计时器组件。挂载和更新时需要用到生命周期函数。

```js
ComponentDidMount(){//组件渲染完成时触发
  document.title = 	`click${this.porps.count}`
};
...
class组件内一个简单的计时器实现需要两个生命周期函数，并且用同样的代码实现了同样的功能。

而使用Hook能将相同的逻辑抽离出来。

```js
useEffect(()=>{setCount(count+1)},[conut])
```
